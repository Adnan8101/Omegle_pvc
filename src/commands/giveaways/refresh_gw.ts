...
export default {
    ...
    async run(ctx: any, targetArg?: string, messageIdArg?: string) {
        ...
        try {
            ...
            // Fetch all giveaways (both active and ended)
            ...
            for (const giveaway of giveaways) {
                try {
                    ...
                    if (giveaway.ended) {
                        // For ended giveaways, update with winner information
                        const winners = await prisma.giveawayWinner.findMany({
                            where: { giveawayId: giveaway.id },
                            select: { userId: true }
                        });
                        ...
                        // For active giveaways, update with current information
                        let participantCount = await prisma.giveawayParticipant.count({
                            where: { giveawayId: giveaway.id }
                        });
